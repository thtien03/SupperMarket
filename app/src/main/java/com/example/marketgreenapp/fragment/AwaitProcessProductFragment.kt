...
class AwaitProcessProductFragment : Fragment() {
    ...
    private var awaitProcessProductFragmentBinding: FragmentAwaitProcessProductBinding? = null
    private val binding get() = awaitProcessProductFragmentBinding!!
    ...
    override fun onCreateView(
        ...
        if (awaitProcessProductFragmentBinding == null)
            awaitProcessProductFragmentBinding =
                FragmentAwaitProcessProductBinding.inflate(layoutInflater)

        awaitProcessProductFragmentBinding!!.lottieLoadingData.visibility = View.VISIBLE
        lifecycleScope.launch(Dispatchers.Main) {
            ...
            if (currentUserDataStoreManager != null) {
                ...
                println("=========================${currentUserDataStoreManager.uid}")
                awaitProcessProductFragmentBinding!!.lottieLoadingData.cancelAnimation()
                awaitProcessProductFragmentBinding!!.lottieLoadingData.visibility = View.GONE
                    ...
                    for (snapshotChildren in dataSnapshot.children) {
                        ...
                        productStatus?.let {
                            if (it.nameUserId.equals(currentUserDataStoreManager.uid)) {
                                if (it.orderStatus.equals(OrderStatus.PENDING)) {
    ...
    private fun setDataToRecyclerView() {
            ...
            object : ProductAwaitProcessAdapter.IOnClickDeliveryAddress {
                override fun onClickDelivery(productStatus: ProductStatus, position: Int) {
                    ...
                    val pairs: Array<Pair<View, String>> = arrayOf(
                        Pair(awaitProcessProductFragmentBinding!!.layoutRoot, "transition_drawer")
                    )
                ...
                override fun onClickProductDetail(product: ProductStatus, position: Int) {
                    ...
                    TransitionHelper.navigateWithTransition(
                        ...
                        awaitProcessProductFragmentBinding!!.layoutRoot,
                        "transition_drawer",
                        R.anim.slide_in_right,
                        R.anim.slide_no_anim,
                        bundle
        ...
        awaitProcessProductFragmentBinding!!.rcvProductAwaitProcess.setHasFixedSize(false)
        awaitProcessProductFragmentBinding!!.rcvProductAwaitProcess.layoutManager =
            LinearLayoutManager(requireContext())
        val dividerItemDecoration = DividerItemDecoration(requireContext(), RecyclerView.VERTICAL)
        awaitProcessProductFragmentBinding!!.rcvProductAwaitProcess.addItemDecoration(
            dividerItemDecoration
        )
        awaitProcessProductFragmentBinding!!.rcvProductAwaitProcess.adapter =
    ...
    private fun shopSolutionUpdateProductStatus(productStatus: ProductStatus, position: Int) {
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                if (productStatus.orderStatus.equals(OrderStatus.PENDING)) {
                    val productOrderRef = MarketGreenFirebaseApp[requireContext()]
                        .getDataProductOrderFromFirebaseDatabaseReference()
    ...
    override fun onDestroy() {
        super.onDestroy()
        awaitProcessProductFragmentBinding = null
    }
}